...
# root directory, while documentation files are spread around
# the tree.
...
# Represents the content which must be extracted from the project tree,
...
def src_deps(project_root, src_file, dest, src_root):
    # - project_root: the project_root directory containing src_file
    ...
    # dependency's relative path from project_root.
    ...
    dst_dir = path.join(dest, path.relpath(src_dir, start=project_root))
...
def find_content(project_root, src, dest, fnfilter, ignore, src_root):
    ...
    outputs = []
    output_dirs = set()
    for dirpath, dirnames, filenames in os.walk(path.join(project_root, src)):
        ...
        # needs to exist.
        dst_dir = path.join(dest, path.relpath(dirpath, start=project_root))
        ...
        for src_rel in sources:
            ...
            deps = src_deps(project_root, src_abs, dest, src_root)
...
def main():
    parser = argparse.ArgumentParser(
        description='''Recursively copy documentation files from project root to
        a destination folder, along with files referenced in those .rst files
        by a configurable list of directives: {}.'''.format(DIRECTIVES))
    ...
    for fnfilter, source, dest in content_config:
        content = find_content(args.project_root, source, dest, fnfilter, ignore,
